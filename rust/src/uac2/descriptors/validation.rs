...
pub fn validate_iad(iad: &Iad) -> Result<(), Uac2Error> {
    if iad.b_interface_count == 0 {
        return Err(Uac2Error::InvalidDescriptor("b_interface_count must be > 0".to_string()));
    }
    Ok(())
}
...
pub fn validate_ac_interface_header(h: &AcInterfaceHeader) -> Result<(), Uac2Error> {
    if h.w_total_length == 0 {
        return Err(Uac2Error::InvalidDescriptor("w_total_length must be > 0".to_string()));
    }
    Ok(())
}
...
pub fn validate_input_terminal(t: &InputTerminal) -> Result<(), Uac2Error> {
    if t.b_terminal_id == 0 {
        return Err(Uac2Error::InvalidDescriptor("b_terminal_id must be non-zero".to_string()));
    }
    Ok(())
}
...
pub fn validate_feature_unit(f: &FeatureUnit) -> Result<(), Uac2Error> {
    if f.b_unit_id == 0 {
        return Err(Uac2Error::InvalidDescriptor("b_unit_id must be non-zero".to_string()));
    }
    if f.b_control_size != 4 {
        return Err(Uac2Error::InvalidDescriptor("b_control_size must be 4".to_string()));
    }
    Ok(())
}
...
pub fn validate_as_interface_general(g: &AsInterfaceGeneral) -> Result<(), Uac2Error> {
    if g.w_format_tag == 0 {
        return Err(Uac2Error::InvalidDescriptor("w_format_tag must be non-zero".to_string()));
    }
    Ok(())
}
