...
mod uac2_performance {
    ...
    fn test_audio_pipeline_latency() {
        ...
        let pipeline = AudioPipeline::new(format.clone(), format, 8192).unwrap();
    ...
    fn test_passthrough_performance() {
        ...
        let pipeline = AudioPipeline::new(format.clone(), format, 16384).unwrap();
